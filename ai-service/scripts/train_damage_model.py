...
DEFAULT_DATA = PROJECT_ROOT / "datasets" / "vehicle_damage_detection_v2" / "data.yaml"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Train the EksperSiz Damage Detection V2 experiment."
    )
...
def train_damage_model(args: argparse.Namespace) -> None:
    data_path = args.data.resolve()
    model_path = args.model.resolve()
    if not model_path.is_file():
        raise FileNotFoundError(f"Pretrained detection model not found: {model_path}")
...
def main() -> None:
    train_damage_model(build_parser().parse_args())
